...
排序与解析（近期重点）
- 优化剧集排序（24259d51）：修复 01光的继承者.flv 这类集号紧贴中文的剧集排序错乱问题，7 个网盘 Sorter 的 fallback 改为按原始文件名排序
- 剧集排序优化（180e6ff7）：集号识别逻辑大改，支持 S01E02/EP02/第02集/02集 等格式
- 修复自有盘的排序（78493ab9）
- douban 改版（f884b439）：
网盘相关
- 修复 shareId 缓存问题
...
3.优化星剧社的显示
4.优化网盘顺序的显示
5.新增光呀社区、蜗牛（翻译水友一天给的文件）


1.新增services服务配置，可以运行termux相关服务，比如直接本地运行danmu-api

1.修复youtube、兼容更多壳子
2.新增termux配置，可以配置各种环境、各种软件，比如nodejs、clash等等，详见services.json
...
2.优化夸克、uc ck读取
3.修复bili分类问题
4.新增瓜子、4k指南、爱看机器人、观影
5.修复一些已知问题

1.修复金牌、游戏直播
2.优化goProxy加载顺序，优先加载64位
3.优化goProxy逻辑
4.提高emby兼容



1.完善bili，可直接跳转bili app授权
2.增加pansoufish,适配根据关键词返回结果
3.播放页面 优化推送按钮
4.修复游戏直播
5.优化goProxy验证机制

2.修复youtube播放困难，也不再需要ck;
...
1.修复proxy留空，默认会设置1072的问题
2.兼容小雅-emby
3.singbox 适配更多订阅链接导入

近期更新
0.新增emby，兼容各种emby服，有回传和视轨，加了一些自定义的配置 需要梯子的请配置代理头 比如 http://127.0.0.1:9966/api/proxy/emby地址 代理头用的就是proxy配置的代理
1.放宽goProxy验证规则 比如首页是采集非网盘源切回到网盘源不再校验
2.goProxy优先访问ipv6地址(改善夸克夜间播放体验)
3.不再需要exeMd5
4.优化goProxy的梯子代理能力，代理配置：http://127.0.0.1:9966/api/proxy/需要梯子访问的地址，目前差不多可以代理一切
5.兼容521影视
6.新增本地字幕推送，主要方便把手机上的字幕文件推送到电视 地址：127.0.0.1:8877/proxy?do=push 也是推送输入文本的地址，可以直接手机扫电视上的二维码（配置中心点开某项输入框，会显示二维码），只适配了影视。
7.搜索后播放不了115，是因为网盘合集源里有很多115的分享，一起搜索会触发风控，需要关闭网盘合集的搜索或者删掉115合集即可
8.优化123盘
9.修复玩偶-虎斑
更多使用信息参阅https://t.me/juejijianghu 发布频道
             https://t.me/+U1KVb2FPkUNkYTM9 聊天频道

1.夸克、uc支持直接连接app获取ck,不需要扫码
2.登陆版本tgsou支持私密频道搜索，自动识别群组和频道去搜索，大大减少风控。
3.兼容123新的分享格式（tgsou所有相关的）
4.修复tgsou频道图片显示（图片显示要梯子 要么是服务器的 要么是本地梯子）
5.优化了其他不知道的
